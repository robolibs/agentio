...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    thread::spawn(move || loop {
        match rpc_srv.recv_timeout(Duration::from_millis(100)) {
            Ok(Some((sample, reply))) => {
                ...
                println!("  [Req/Res] Generating and responding with {:.2} MB payload...", req_bytes as f64 / 1_000_000.0);
    ...
    thread::spawn(move || loop {
        match que_srv.recv_timeout(Duration::from_millis(100)) {
            Ok(Some((que_sample, mut ans_sender))) => {
                ...
                println!("  [Que/Ans] Streaming {} chunks of {:.2} MB each...", q.total_chunks, q.chunk_bytes as f64 / 1_000_000.0);
    ...
    // ------------------------------------------------------------------------
    let mut put_srv = agent.put_server::<HeavyUploadBlock, HeavyUploadAck>("/heavy/upload_blocks")?;
    thread::spawn(move || loop {
        match put_srv.recv_timeout(Duration::from_millis(100)) {
            Ok(Some(mut puts_recv)) => {
                ...
                while let Ok(Some(block)) = puts_recv.next() {
                    ...
                    if blocks % 10 == 0 {
                        println!("  [Put/Ack] Progress: Received {} blocks ({:.2} MB)", blocks, total_bytes as f64 / 1_000_000.0);
                    }
                }
                println!("  [Put/Ack] Upload complete: {} blocks ({:.2} MB). Sending Ack...", blocks, total_bytes as f64 / 1_000_000.0);
